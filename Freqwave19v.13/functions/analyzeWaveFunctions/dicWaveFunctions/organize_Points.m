function [sorted, meshPts] = organize_Points(meshPts, valid_tracked, u, v)
...
for i=1:1:u-1
    for j=1:1:v-1
        ...
        for k=length(valid_tracked(:,1)):-1:1
            % tan range -pi:pi need to add conditionals to range 0:2pi
            ...
            theta2 = calculate_Angle((end_points(k,1)-meshBox(4,1)),(end_points(k,2)-meshBox(4,2)));
            ...
            % theta check
            elseif (between1 == 1 && between2 == 1) 
                if ((theta1 >= min(theta1_2,theta1_3) && theta1 <= max(theta1_2,theta1_3))...
                        && (theta2 >= min(theta4_2,theta4_3) && theta2 <= max(theta4_2,theta4_3)))
                    boxPoints = [boxPoints; end_points(k,:)];
                end
            elseif (between1 == 1 && between2 == 0)
                if ((theta1 >= min(theta1_2,theta1_3) | theta1 <= max(theta1_2,theta1_3))...
                        && (theta2 <= min(theta4_2,theta4_3) && theta2 >= max(theta4_2,theta4_3)))
                    boxPoints = [boxPoints; end_points(k,:)];
                end
            elseif (between1 == 0 && between2 == 1)
                if ((theta1 <= min(theta1_2,theta1_3) | theta1 >= max(theta1_2,theta1_3))...
                        && (theta2 >= min(theta4_2,theta4_3) && theta2 <= max(theta4_2,theta4_3)))
                    boxPoints = [boxPoints; end_points(k,:)];
                end
            elseif (between1 == 0 && between2 == 0)
                if ((theta1 <= min(theta1_2,theta1_3) && theta1 >= max(theta1_2,theta1_3))...
                        && (theta2 <= min(theta4_2,theta4_3) && theta2 >= max(theta4_2,theta4_3)))
                    boxPoints = [boxPoints; end_points(k,:)];
                end
