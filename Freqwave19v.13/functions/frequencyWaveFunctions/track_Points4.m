function [all_tracked, point_validity] = track_Points4(pointTrackerArray, images, initial_points, startFrame, endFrame,rote)
...
for i = 1:1:length(pointTrackerArray)
    ...
    while index<=(endFrame) % start at 2 because first image already read
        ...
        %%% add jump check
%         if norm(points-c(index-offset-1,:)) >= 10
%             point_validity = 0;
%         end
        %%%
        if abs(norm(points-c(index-offset-invalid_count,:))) >= 10 || point_validity == 0
